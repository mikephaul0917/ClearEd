...
export default function EmailPolicyGuidePage() {
  ...
  const sections = [
    {
      h: "1. The Official Domain Requirement",
      p: "ClearEd operates on a strict 'Official Domains Only' policy. This means that all users—whether students, officers, or administrators—must register using an email address issued by their respective academic institution (e.g., @school.edu or @university.edu.ph)."
    },
    {
      h: "2. Why We Restrict Personal Domains",
      p: "Personal email providers like Gmail, Yahoo, and Outlook do not allow us to cryptographically verify your affiliation with an institution. By restricting registration to official domains, we ensure that student records remain secure and accessible only to authorized personnel within that specific school network."
    },
    {
      h: "3. Automatic Blocking System",
      p: "Our system maintains an active filter that automatically blocks registration attempts from public email services. This 'Zero-Trust' approach is fundamental to protecting institutional integrity and preventing unauthorized data access."
    },
    {
      h: "4. Institutional Domain Whitelisting",
      p: "For an institution to be active on ClearEd, its specific email domain must first be whitelisted by our Super Admins. This multi-layered verification process ensures that once a domain is active, it represents a legitimate and vetted academic entity."
    },
    {
      h: "5. What to do if your domain is missing",
      p: "If your institution uses a custom domain that is not currently recognized by our system, your administrator must contact our support team. We will perform a manual verification of the domain's ownership before adding it to our secure whitelist."
    }
  ];
          ...
          <Box display="flex" alignItems="center" gap={4}>
            {navLinks.map((link) => (
              <Typography 
                ...
                sx={{ 
                  fontWeight: 600, fontSize: "14px", color: C.black, opacity: link.path === "/guides/email-policy" ? 1 : 0.6, cursor: "pointer",
                  "&:hover": { opacity: 1 }
                }}
              >
                {link.label}
